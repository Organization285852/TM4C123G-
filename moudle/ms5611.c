...
//    IIC_Write_1Byte(MS5611_ADDR, CMD_RESET, 1); 
	I2CWriteToReg(MS5611_ADDR,CMD_RESET,&b,1);
}
...
u8 MS5611_Read_Prom(void)
{
	...
	for (i = 0; i < PROM_NB; i++)
	{
		check += I2CReadToBuf(MS5611_ADDR, CMD_PROM_RD + i * 2,rxbuf,2); // send PROM READ command
...
{
	I2CReadToBuf( MS5611_ADDR, CMD_ADC_READ, t_rxbuf,3 ); // read ADC
}

void MS5611_Read_Adc_P(void)
{
	I2CReadToBuf(MS5611_ADDR, CMD_ADC_READ,p_rxbuf,3); // read ADC
}
...
//	IIC_Write_1Byte(MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D2 + MS5611_OSR, 1); // D2 (temperature) conversion start!
    I2CWriteToReg(MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D2 + MS5611_OSR, &b,1);
}
...
//  IIC_Write_1Byte(MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D1 + MS5611_OSR, 1); // D1 (pressure) conversion start!
	I2CWriteToReg(MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D1 + MS5611_OSR,&b,1);
}
...
void MS5611_BaroAltCalculate(void)
{
  ...
  int32_t pressure;
	...
	int64_t sens;
		...
		static volatile int32_t sum_tmp_5611 = 0;//__IO int32_t
